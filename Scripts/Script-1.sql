...
/*보드테이블과 보드리플라이테이블을 보드의보드넘과 보드리플라이의 보드넘으로 레프트아우터 조인을하고 보드리프*/
SELECT * FROM board LEFT OUTER JOIN BOARD_REPLY ON board.board_no = board_reply.board_no LEFT OUTER JOIN MEMBER1  ON board_reply.BOARD_REPLY_MEMBER_ID = member1.member_id where board.board_no =4 order by board_reply.board_reply_no desc ;
SELECT * FROM board LEFT OUTER JOIN BOARD_REPLY ON board.board_no = board_reply.board_no  JOIN MEMBER1  ON board.member_id = board_reply.board_reply_member_id where board.board_no = 4 order by board_reply.board_reply_no DESC;
